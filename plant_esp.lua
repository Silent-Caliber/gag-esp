...
local function createSizeToggleBtn(frame)
    ...
    -- Create container frame for button
    local btnContainer = Instance.new("Frame", frame)
    btnContainer.Name = "SizeToggleBtn"
    btnContainer.Size = UDim2.new(0, 30, 0, 30)
    btnContainer.Position = UDim2.new(1, -36, 0, 2)
    btnContainer.BackgroundTransparency = 1
    btnContainer.ZIndex = 101

    -- Background frame for press effect
    local bg = Instance.new("Frame", btnContainer)
    bg.Name = "Background"
    bg.Size = UDim2.new(1, 0, 1, 0)
    bg.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    bg.BackgroundTransparency = 0.15
    ...
    -- Add border
    local sizeToggleStroke = Instance.new("UIStroke", bg)
    ...
    btn.Image = "rbxassetid://103380019858869"
    btn.ZIndex = 100
    ...
    local function setPressedState(pressed)
        TweenService:Create(
            bg,
            TweenInfo.new(0.1),
            {BackgroundTransparency = pressed and 0.05 or 0.15}
        ):Play()
    end
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        -- Adjust UI sizes for compact mode
        if compact then
